    ...
    for batch_idx = 1:batch_num
        ...
        for i = 2:layer*2+1
            if isPCA == 1 && i == layer+1
                ...
                In{i} = W{i-1} * Out{i-1};
                Out{i} = [1./(1 + exp(-In{i}));ones(1,batch_size)];
            end
        end
        loss = 0.5 * sum(sum((Out{2*layer+1}(1:hidden_num(1),:) - In{1}).^2)) ...
            + lambda * 0.5 * sum(sum(Out{layer+1}(1:hidden_num(layer+1),:).^2));
        disp (loss);
        % feed back
        delta = NaN;
        for i = 2*layer : -1: 1
            if isnan(delta)
                delta = 1/batch_size .* (Out{i+1}(1:hidden_num(1),:) - In{1}) .* Out{i+1}(1:hidden_num(1),:) .* (1-Out{i+1}(1:hidden_num(1),:));
            elseif i == layer && isPCA == 1
                delta = W{i+1}' * delta;
                delta = delta(1:size(delta,1)-1,:);
            else
                delta = W{i+1}' * delta;
                delta = delta(1:size(delta,1)-1,:) .* Out{i+1}(1:size(delta,1)-1,:) .* (1-Out{i+1}(1:size(delta,1)-1,:));
            end
            dW{i} = delta * Out{i}';
        end
        delta = NaN;
        for i = layer:-1:1
            if isnan(delta)
                if isPCA == 0
                    delta = lambda * Out{layer+1}(1:hidden_num(layer+1),:) .* Out{layer+1}(1:hidden_num(layer+1),:) .* (1-Out{layer+1}(1:hidden_num(layer+1),:));
                else
...
for i = 2:layer*2+1
    if isPCA == 1 && i == layer+1
        ...
        In{i} = W{i-1} * Out{i-1};
        Out{i} = [1./(1 + exp(-In{i}));ones(1,num_show)];
    end
end
